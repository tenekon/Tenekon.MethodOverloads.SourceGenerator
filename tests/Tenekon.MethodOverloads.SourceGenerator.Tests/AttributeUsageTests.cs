...
public sealed class AttributeUsageTests
{
    ...
    public void Attribute_usage_matches_acceptance_targets()
    {
        ...
        var usageByAttribute = new Dictionary<string, HashSet<AcceptanceTestData.AttributeTargetKind>>(StringComparer.Ordinal);
        ...
        {
            Assert.True(targetsByAttribute.TryGetValue(attributeName, out var expectedTargets),
                $"Expected acceptance criteria targets for {attributeName}.");
            Assert.True(usageByAttribute.TryGetValue(attributeName, out var actualTargets),
    ...
    private static Dictionary<string, HashSet<AcceptanceTestData.AttributeTargetKind>> ExtractAttributeUsages(string source)
    {
