...
public sealed class AcceptanceDiagnosticsTests
{
    ...
    public void Expected_diagnostics_are_reported(DiagnosticCaseResult caseResult)
    {
        ...
        {
            if (caseResult.ActualIds.Count == 0)
            {
                return;
            }
        ...
        var missing = caseResult.ExpectedIds.Except(caseResult.ActualIds).OrderBy(x => x, StringComparer.Ordinal).ToArray();
        var extra = caseResult.ActualIds.Except(caseResult.ExpectedIds).OrderBy(x => x, StringComparer.Ordinal).ToArray();
