...
{
    public static TypeTargetInput? CreateTypeTarget(GeneratorAttributeSyntaxContext attributeContext, CancellationToken cancellationToken)
    {
        if (attributeContext.TargetSymbol is not INamedTypeSymbol typeSymbol)
        {
            return null;
        }
    ...
    public static MethodTargetInput? CreateMethodTarget(GeneratorAttributeSyntaxContext attributeContext, CancellationToken cancellationToken)
    {
        if (attributeContext.TargetSymbol is not IMethodSymbol methodSymbol)
        {
            return null;
        }
        ...
        var methodModel = BuildMethodModel(methodSymbol, cancellationToken, out var syntaxOptions, out var optionsFromAttribute);
    ...
    {
        if (typeTargets.IsDefaultOrEmpty && methodTargets.IsDefaultOrEmpty)
        {
            return null;
        }
        ...
        foreach (var input in typeTargets)
        {
            ...
            else
            {
                ...
                typeTargetsByDisplay[input.Type.DisplayName] = existingTarget with { MatcherTypeDisplays = mergedMatchers };
            }
    ...
    {
        if (!typesByDisplay.ContainsKey(type.DisplayName))
        {
            typesByDisplay[type.DisplayName] = type;
        }
    }

    private static void AddType(Dictionary<string, TypeModel> typesByDisplay, string displayName, TypeModel type)
    {
        if (!typesByDisplay.ContainsKey(displayName))
        {
            typesByDisplay[displayName] = type;
        }
    }
